...
const DESKTOP_APP_ID = "com.royalaid.scriptorium";
const SCRIPTORIUM_PRODUCT_NAME = "Royal's Scriptorium";
const SCRIPTORIUM_ARTIFACT_PREFIX = "Royals-Scriptorium";
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? `${SCRIPTORIUM_PRODUCT_NAME} (Nightly)`
    : (desktopPackageJson.productName ?? SCRIPTORIUM_PRODUCT_NAME);
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: `${SCRIPTORIUM_ARTIFACT_PREFIX}-${version}-\${arch}.\${ext}`,
    directories: {
  ...
  if (platform === "mac") {
    buildConfig.mac = {
      ...
      protocols: [
        {
          name: SCRIPTORIUM_PRODUCT_NAME,
          schemes: ["royal-scriptorium", "royal-scriptorium-dev"],
        },
      ],
      ...(macPasskeySigning
        ? {
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "royals-scriptorium",
      icon: "icons",
      category: "Development",
      desktop: {
        entry: {
          StartupWMClass: "royals-scriptorium",
        },
      },
    };
  }
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    name: "royals-scriptorium",
    version: appVersion,
    buildVersion: appVersion,
    t3codeCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Royal's Scriptorium desktop build",
    author: "Royal Aid",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for Royal's Scriptorium."),
