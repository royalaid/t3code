...
describe("V2 environment commands", () => {
  ...
  it.effect("forwards the expected branch for optimistic metadata updates", () =>
    Effect.gen(function* () {
      const commands: OrchestrationV2Command[] = [];
      const supervisor = yield* makeSupervisor({ commands, projects: [] });

      yield* updateThreadMetadata({
        commandId: CommandId.make("optimistic-branch-update"),
        threadId: v2ThreadId,
      ...
      expect(commands).toEqual([
        {
          ...
          commandId: "optimistic-branch-update",
          threadId: v2ThreadId,
          branch: "t3code/temporary",
          expectedBranch: "t3code/temporary",
        },
