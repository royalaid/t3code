...
describe("orchestration V2 contracts", () => {
  ...
  it("decodes optimistic branch metadata updates", () => {
    const command = decodeOrchestrationV2Command({
      type: "thread.metadata.update",
      commandId: "command-branch-update-1",
      threadId: "thread-1",
      branch: "t3code/temporary",
      expectedBranch: "t3code/temporary",
    });

    expect(command.type).toBe("thread.metadata.update");
    if (command.type !== "thread.metadata.update") {
      throw new Error("expected thread.metadata.update");
    }
    expect(command.expectedBranch).toBe("t3code/temporary");
  });
