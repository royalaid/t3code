...
Before publishing a graph, call goal_read for goal ${goalId}, then call goal_capabilities. Use only advertised protocol capability tokens in node requiredCapabilities; ordinary task skills such as git, markdown, editing, or testing are not protocol capabilities and use requiredCapabilities: []. Use the current goal revision as the compare-and-swap expected revision and publish the complete next graph with goal_replace_graph. Do not finish the turn without publishing a valid graph.

After goal_replace_graph accepts the graph, immediately call goal_read again and confirm the new revision is authoritative. Do not claim that work started merely because publication succeeded; work has started only after the re-read shows a running attempt.
